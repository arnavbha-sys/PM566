...
- Office hours: 3pm Wed. OR 2pm Thur. / SSB 202V
...
![](img/data-science-2.png){fig-align="center"}
...
We'll explore how to work with all of these throughout the semester! But for now, we'll focus on "tidy" data.

---

# Tidy Data

We use the term "tidy data" to refer to tabular datasets that are easily
represented by a grid of values. Each row represents a single observational unit
...
One of the oldest and most common ways to store tabular data on a computer is
with a file type called "comma-separated values" or CSV. These files typically
use the suffice `.csv` and they store one observation per line, with columns
separated by commas.
...
Looking ahead to working with R, we will typically be working with objects
called "data frames".
